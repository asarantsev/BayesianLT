...
print('MStd of real earnings growth = ', round(stdREG, 5))
...
trend_coeff = Reg.params[3] #minus beta times long-term implied dividend yield
Residuals = idivyield - Reg.predict(Y) #regression residuals
stderr = math.sqrt((1/(N-5))*np.dot(Residuals, Residuals)) #standard error for regression residuals
...
sigma = math.sqrt((REG_coeff*stdREG)**2 + stderr**2)

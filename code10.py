...
FT = F.transpose()
Gram = FT.dot(F)
invMatrix_Dev = np.linalg.inv(Gram)# inverting this matrix for Bayesian posterior simulation
...
for sim in range(10):
    print('sim = ', sim)
    simVar, simCoeff = BayesSimMulti(devPointEstimate, devV_hat, invMatrix_Dev, N-5)
    print('Variance = ', simVar)
    print('Coefficients = ', simCoeff)
